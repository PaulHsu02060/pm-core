...
// 會議週曆截圖 → Gemini 視覺抽會議（比 tesseract 準·共用 App.geminiVision·§21.16）·回傳格式對齊 parseMeetingText：{date,startTime,endTime,title}
App._geminiMeetings = async function(dataUrl) {
  const prompt = '這是一張「週行事曆／會議行程表」截圖。抽取每一個有明確「開始時間」的會議或行程。\n規則：\n1. 只抽有開始時間的行程；略過全天標題、週次抬頭、純日期列、空格。\n2. weekday=星期幾，回「週一」「週二」「週三」「週四」「週五」「週六」「週日」其中之一（依該行程所在的欄）。\n3. startTime/endTime=24 小時制 HH:MM；沒有結束時間就回空字串。\n4. title=行程/會議名稱。\n5. 找不到就回 meetings:[]。';
  const schema = { type: 'OBJECT', properties: { meetings: { type: 'ARRAY', items: { type: 'OBJECT', properties: { weekday: { type: 'STRING' }, startTime: { type: 'STRING' }, endTime: { type: 'STRING' }, title: { type: 'STRING' } } } } } };
  const out = await App.geminiVision([dataUrl], prompt, schema);
  const WD = { '週日': 0, '週一': 1, '週二': 2, '週三': 3, '週四': 4, '週五': 5, '週六': 6 };
  const monday = D.monday();
  return (out.meetings || []).filter(m => m && String(m.startTime || '').trim()).map(m => {
    let wl = String(m.weekday || '').replace('星期', '週').replace('禮拜', '週');
    let idx = WD[wl]; if (idx == null) { const mm = wl.match(/[日一二三四五六]/); idx = mm ? WD['週' + mm[0]] : null; }
    let date = '';
    if (idx != null) { const dow = idx === 0 ? 6 : idx - 1; date = D.fmt(D.addDays(monday, dow), 'iso'); }   // 本週該星期的日期（週偏移由 runOCR 依 shot.week 再套）
    return { date: date, startTime: String(m.startTime || '').trim(), endTime: String(m.endTime || '').trim(), title: String(m.title || '').trim() };
  });
};
...
App.runOCR = async function() {
  ...
  try {
    // 有 Gemini 金鑰→AI 視覺辨識（準·跳過 tesseract 載入）；無→tesseract OCR
    const useAI = !!(typeof Materials !== 'undefined' && Materials._hasGemini && Materials._hasGemini());
    App.openLoadingModal('會議截圖辨識', useAI ? 'AI 辨識中 …請稍候（約十幾秒）' : 'OCR 辨識中 …請稍候');
    if (!useAI && !window.tesseractWorker) {
    ...
    for (const shot of this.shotFiles) {
      if (btn) btn.textContent = `⏳ ${useAI ? 'AI ' : ''}辨識中 (${++done}/${total})...`;
      App.setLoadingMsg(`${useAI ? 'AI ' : ''}辨識中 (${done}/${total}) …請稍候`);
      try {
        let meetings;
        if (useAI) {
          try { meetings = await App._geminiMeetings(shot.dataUrl); }
          catch (ge) {   // AI 失敗→退回 tesseract
            console.error('Gemini meeting failed, fallback OCR', ge);
            if (!window.tesseractWorker) { if (btn) btn.textContent = '⏳ 載入語言檔…'; window.tesseractWorker = await Tesseract.createWorker(['chi_tra', 'eng']); }
            const procUrl = await App._preprocessForOcr(shot.dataUrl);
            const { data: { text } } = await window.tesseractWorker.recognize(procUrl); allRaw.push(text);
            meetings = parseMeetingText(text);
          }
        } else {
    ...
    App.closeModal();   // 關載入彈窗·結果顯示在下方 #ocrResult（非 modal）
    if (unique.length === 0) {
