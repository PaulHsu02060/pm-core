...
// 工作檯工具列「設定 ▾」下拉（收：管理會議＋顯示設定）：複用 header 下拉機制（.hdr-menu / closeHdrMenus / _ensureHdrMenuClose 外點關閉）
App.toggleWsSettingsMenu = function(ev) {
  ev.stopPropagation();
  this._ensureHdrMenuClose();
  const m = document.getElementById('wsSettingsMenu');
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (item) {
        ...
        if (task) {
          ...
          const isOverdue = isTaskDelayed(task, today);   // §4.6 逾期單一口徑（排除 done+hold·有效迄日）；原手寫漏排除 hold（Paul 2026-07-05 收斂）
          // Tooltip
        ...
        if (meetingAuto.isFirstSlot) {
          ...
          html += `<div class="ws-event meeting ${cssClass}" style="top:${_aTop}px; height:${_aH}px; z-index:var(--z-ws-event-low);" title="${U.esc(tip)}">
    ...
    onSubmit: (newText) => {
      ...
      if (!trimmed) {
        App.confirmModal({ icon: 'ti-trash', iconBg: '--rose-l', iconColor: '--rose-ink',
          ...
          onConfirm: () => { DATA.memos = DATA.memos.filter(m => m.id !== id); Store.memos.save(); Workspace.render(); U.toast('✓ 已刪除'); } });
        return;
      }
