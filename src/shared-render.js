...
App.renderGantt=function(targetId="page-gantt",singleProject=false){this.ganttScope={targetId,singleProject};if(this.ganttFilterOpen===void 0)this.ganttFilterOpen=false;if(!this.ganttStart)this.ganttStart=D.monday();if(!this.ganttProjectFilter)this.ganttProjectFilter=new Set(DATA.projects.map(p=>p.id));const start=this.ganttStart;const days=[];for(let i=0;i<14;i++)days.push(D.addDays(start,i));const endDay=days[13];const today=D.today();const wd=["日","一","二","三","四","五","六"];let headerHtml='<div class="gantt-corner">任務</div>';for(const d of days){const isHol=!D.isWorkday(d);const isToday=D.isSameDay(d,today);const holName=(()=>{if(!isHol)return"";const hols=DATA.calendars?.base?.holidays||{};const selfKey=D.fmt(d,"iso");if(hols[selfKey])return hols[selfKey].slice(0,2);for(let back=1;back<=4;back++){const prev=new Date(d);prev.setDate(prev.getDate()-back);if(D.isWorkday(prev))break;const prevKey=D.fmt(prev,"iso");if(hols[prevKey])return hols[prevKey].slice(0,2)}return""})();headerHtml+=`<div class="gantt-day-header ${isHol?"holiday":""} ${isToday?"today":""}">
    ...
    </div>`}const projFilter=this.ganttProjectFilter;const tasks=DATA.tasks.filter(t=>{if(t._deleted)return false;if(!projFilter.has(t.project))return false;if(this.ganttStageFilter&&!this.ganttStageFilter.has(normStage(t.stage)))return false;if(this.ganttOwnerFilter&&!App._ganttOwnersOf(t).some(o=>this.ganttOwnerFilter.has(o)))return false;const sch=getEffectiveSchedule(t);if(!sch.start&&!sch.end)return false;const ts=sch.start?new Date(sch.start):new Date(sch.end);const te=sch.end?new Date(sch.end):new Date(sch.start);return D.daysBetween(start,te)>=0&&D.daysBetween(ts,endDay)>=0});if(tasks.length===0){document.getElementById(targetId).innerHTML=`
  ...
  `;if(singleProject)this._drawGanttLinks(targetId)};App.buildGanttHeaderHtml=function(days){const periodStr=D.fmtRange(days[0],days[13]," – ");return`<div class="gantt-header-row">
  ...
  </div>`};App.ganttShift=function(days){this.ganttStart=D.addDays(this.ganttStart||D.monday(),days);this.renderGantt(this.ganttScope.targetId,this.ganttScope.singleProject)};App.ganttToday=function(){this.ganttStart=D.monday();this.renderGantt(this.ganttScope.targetId,this.ganttScope.singleProject)};const GANTT_NO_OWNER="未指派";App._ganttOwnersOf=function(t){const list=(t&&t.owner||"").split(/[、\/＋+]/).map(s=>s.trim()).filter(Boolean);return list.length?list:[GANTT_NO_OWNER]};App._ganttFilterPool=function(){const pf=this.ganttProjectFilter;const tasks=DATA.tasks.filter(t=>!t._deleted&&(!pf||!pf.size||pf.has(t.project)));return{tasks,stages:[...new Set(tasks.map(t=>normStage(t.stage)))],owners:[...new Set(tasks.flatMap(t=>App._ganttOwnersOf(t)))].sort()}};App.buildGanttFilterHtml=function(singleProject){const _pool=App._ganttFilterPool();const stages=_pool.stages;const owners=_pool.owners;const sf=this.ganttStageFilter;const of_=this.ganttOwnerFilter;const pf=this.ganttProjectFilter||new Set;const projOpen=this.ganttFilterOpen;const projMenu=projOpen?`<div class="gantt-filter-menu">
    ${DATA.projects.map(p=>`
    ...
    ${stages.map(s=>`
      <label class="gantt-filter-item">
        <input type="checkbox" ${!sf||sf.has(s)?"checked":""} onchange="App.toggleGanttStage('${U.attrJs(s)}')">
        ${U.esc(s)}
      </label>
    `).join("")}
    ...
    ${owners.map(o=>`
      <label class="gantt-filter-item">
        <input type="checkbox" ${!of_||of_.has(o)?"checked":""} onchange="App.toggleGanttOwner('${U.attrJs(o)}')">
        ${U.esc(o)}
      </label>
    `).join("")}
    ...
    ${ownerMenu}
  </div>`;return`<div class="gantt-filter-bar">${projFilter}${stageFilter}${ownerFilter}</div>`};App.toggleGanttFilterOpen=function(){this.ganttFilterOpen=!this.ganttFilterOpen;this.renderGantt(this.ganttScope.targetId,this.ganttScope.singleProject)};App.toggleGanttProject=function(id){if(!this.ganttProjectFilter)this.ganttProjectFilter=new Set(DATA.projects.map(p=>p.id));if(this.ganttProjectFilter.has(id))this.ganttProjectFilter.delete(id);else this.ganttProjectFilter.add(id);this.renderGantt(this.ganttScope.targetId,this.ganttScope.singleProject)};App.toggleGanttStageOpen=function(){this.ganttStageOpen=!this.ganttStageOpen;this.renderGantt(this.ganttScope.targetId,this.ganttScope.singleProject)};App.toggleGanttOwnerOpen=function(){this.ganttOwnerOpen=!this.ganttOwnerOpen;this.renderGantt(this.ganttScope.targetId,this.ganttScope.singleProject)};App.toggleGanttStage=function(s){const all=App._ganttFilterPool().stages;if(!this.ganttStageFilter)this.ganttStageFilter=new Set(all);if(this.ganttStageFilter.has(s))this.ganttStageFilter.delete(s);else this.ganttStageFilter.add(s);if(this.ganttStageFilter.size===0||this.ganttStageFilter.size===all.length)this.ganttStageFilter=null;this.renderGantt(this.ganttScope.targetId,this.ganttScope.singleProject)};App.toggleGanttOwner=function(o){const all=App._ganttFilterPool().owners;if(!this.ganttOwnerFilter)this.ganttOwnerFilter=new Set(all);if(this.ganttOwnerFilter.has(o))this.ganttOwnerFilter.delete(o);else this.ganttOwnerFilter.add(o);if(this.ganttOwnerFilter.size===0||this.ganttOwnerFilter.size===all.length)this.ganttOwnerFilter=null;this.renderGantt(this.ganttScope.targetId,this.ganttScope.singleProject)};const GANTT_STATUS_LABELS={manual:"手動",override:"鎖",scheduled:"排程"};const GANTT_SOURCE_DESC={manual:"手動錨點",override:"本地鎖定（override）",scheduled:"機器排程連動"};App.buildGanttRowHtml=function(task,start,days,schedById){const proj=this.getProj(task.project);const sch=getEffectiveSchedule(task);const isMilestone=task.taskType==="milestone";const col=d=>D.daysBetween(start,new Date(d));const clampIdx=n=>Math.max(0,Math.min(13,n));const psIdx=isMilestone?col(sch.start||sch.end):col(sch.forecastStart||sch.start||sch.end);let peIdx=isMilestone?psIdx:col(sch.forecastEnd||sch.end||sch.start);if(!isMilestone&&Number.isFinite(psIdx)&&Number.isFinite(peIdx)&&peIdx<psIdx)peIdx=psIdx;let aSIdx=psIdx,aEIdx=peIdx;let fillClass="",showFill=false,overdueDays=0;const todayD=D.today();if(!isMilestone){if(task.status==="done"||task.actualEnd){aSIdx=col(task.actualStart||sch.plannedStart||sch.start||sch.end);aEIdx=col(task.actualEnd||sch.plannedEnd||sch.end);fillClass="fill-done";showFill=true}else if(task.status==="hold"){fillClass="fill-hold";showFill=true}else if(isTaskDelayed(task,todayD)){aSIdx=psIdx;aEIdx=col(todayD);fillClass="fill-over";showFill=true;overdueDays=overdueWorkdays(sch.end,todayD)}else if(task.actualStart){aSIdx=col(task.actualStart);aEIdx=col(todayD);fillClass="fill-wip";showFill=true}}const startCol=Math.max(0,Math.min(psIdx,aSIdx));const endCol=Math.min(13,Math.max(peIdx,aEIdx));const span=endCol-startCol+1;if(!Number.isFinite(startCol)||!Number.isFinite(endCol))return"";if(startCol>13||endCol<0)return"";if(endCol<startCol)return"";const leftPct=idx=>(clampIdx(idx)-startCol)/span*100;const rightPct=idx=>(clampIdx(idx)+1-startCol)/span*100;const r=schedById&&schedById.get(task.id);let statusKey=null;if(r){if(r.anchorSource==="manual")statusKey="manual";else if(r.anchorSource==="override")statusKey="override";else if(!r.blocked&&!r.toSchedule&&!r.error&&r.suggestedStart)statusKey="scheduled"}const hasIssue=!!(r&&(r.error==="circular"||r.blocked||r.toSchedule));const statusTagHtml=statusKey?`<span class="gantt-status-tag tag-${statusKey}">${GANTT_STATUS_LABELS[statusKey]}</span>`:"";const warnHtml=hasIssue?`<span class="gantt-warn" title="排程異常">!</span>`:r&&r.predDelayed?`<span class="gantt-warn gantt-warn-delay" title="${U.esc((r.warnings||[]).join("；"))}">延</span>`:"";const titleLines=[];if(statusKey)titleLines.push(`來源：${GANTT_STATUS_LABELS[statusKey]}（${GANTT_SOURCE_DESC[statusKey]}）`);if(r){if(r.error==="circular")titleLines.push("⚠ 循環依賴：無法排程");else if(r.blocked)titleLines.push("⚠ 受阻：上游尚未排出");else if(r.toSchedule)titleLines.push("⚠ 待排：無前置且未填開始日");if(r.warnings&&r.warnings.length)titleLines.push("提醒："+r.warnings.join("；"))}const barTitle=titleLines.join("|");let html=`<div class="gantt-row-label">
  ...
  </div>`};App.toggleHintBox=function(key){const box=document.querySelector('.hintbox[data-hintkey="'+key+'"]');if(!DATA.settings.hintBoxState)DATA.settings.hintBoxState={};const shown=box?box.classList.contains("collapsed"):!!DATA.settings.hintBoxState[key];DATA.settings.hintBoxState[key]=!shown;Store.settings.save();if(!box)return;const collapsed=!!DATA.settings.hintBoxState[key];box.classList.toggle("collapsed",collapsed);const tg=box.querySelector(".hintbox-toggle");if(tg)tg.textContent=collapsed?"展開▾":"收起▴";const bar=box.querySelector(".hintbox-bar");if(bar){if(collapsed){const title=(box.querySelector(".hintbox-title")||{}).textContent||"";const summary=(box.querySelector(".hintbox-summary")||{}).textContent||"";bar.setAttribute("data-tip",title+"|"+summary+" — 點擊展開看完整說明")}else{bar.removeAttribute("data-tip")}}};
